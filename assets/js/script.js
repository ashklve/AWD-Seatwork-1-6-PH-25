document.addEventListener('DOMContentLoaded', displayHistory);
        ...
        document.getElementById('formGrade').addEventListener('submit', function(event) {
            ...
            document.getElementById('output').innerHTML = `<p><strong>${name}</strong>, your General Weighted Average (GWA) is: <strong>${gwa}</strong></p>`;
        ...
        function displayHistory() {
            ...
            history = history.filter(entry => now - entry.timestamp < 1800000); // 30 minutes
            localStorage.setItem('gwaHistory', JSON.stringify(history));
